...
export const ObjectSettingsDialog: Component<{
  ...
  const saveMetadata = () => {
    ...
    run(
      () =>
        S3Service.UpdateMetadata(
          props.connId,
          props.bucket,
          props.objKey,
          update as any
        ) as Promise<void>,
      t("objectSettings.metadataSaved")
    );
  };
  ...
  const applyAcl = () =>
    run(async () => {
      ...
      const fresh = await S3Service.GetACL(props.connId, props.bucket, props.objKey).catch(
        () => null
      );
  ...
  const doRestore = () =>
    run(
      ...
      t("objectSettings.restoreStarted")
    );
            ...
            <Show when={tab() === "metadata"}>
              <div class="flex flex-col gap-3">
                ...
                <Field label={t("properties.cacheControl")}>
                  <Input
                    value={cacheControl()}
                    onInput={(e) => setCacheControl(e.currentTarget.value)}
                  />
                </Field>
                <Field label={t("objectSettings.contentDisposition")}>
                  <Input
                    value={contentDisposition()}
                    onInput={(e) => setContentDisposition(e.currentTarget.value)}
                  />
                </Field>
                <Field label={t("properties.contentEncoding")}>
                  <Input
                    value={contentEncoding()}
                    onInput={(e) => setContentEncoding(e.currentTarget.value)}
                  />
                </Field>
                ...
                <div class="mt-1">
                  ...
                  <div class="mt-1 flex flex-col gap-2">
                    <For
                      each={meta()}
                      fallback={
                        <p class="text-xs text-muted-foreground">{t("properties.noMetadata")}</p>
                      }
                    >
                      {(m, i) => (
                        <div class="grid grid-cols-[1fr_1fr_auto] gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => setMeta((p) => p.filter((_, idx) => idx !== i()))}
                          >
                    ...
                    <Button
                      ...
                      size="sm"
                      class="self-start"
                      onClick={() => setMeta((p) => [...p, { key: "", value: "" } as Tag])}
                    >
                      <Plus class="h-4 w-4" />
                      {t("tags.add")}
                    </Button>
            ...
            <Show when={tab() === "permissions"}>
              <div class="flex flex-col gap-3">
                  ...
                  <Show
                    ...
                    fallback={
                      <Badge variant="secondary">
                        ...
                        {t("objectSettings.private")}
                      </Badge>
                    }
                  >
                    <Badge variant="destructive">
                      <Globe class="mr-1 h-3 w-3" />
                      {t("objectSettings.public")}
                    </Badge>
                  </Show>
                  <Show when={acl()?.owner}>
                    <span class="text-xs text-muted-foreground">
                      {t("objectSettings.owner")}: {acl()?.owner}
                    </span>
                  </Show>
                </div>
                <Field label={t("objectSettings.cannedAcl")}>
                  <SimpleSelect
                    value={cannedAcl()}
                    options={CANNED_ACLS.map((a) => ({ value: a, label: a }))}
                    onChange={setCannedAcl}
                  />
                </Field>
                <div>
                  ...
                  <div class="mt-1 flex flex-col gap-1">
                    <For
                      each={acl()?.grants ?? []}
                      fallback={<p class="text-xs text-muted-foreground">{t("common.none")}</p>}
                    >
                      {(g) => (
                ...
                <div class="flex justify-end">
                  <Button size="sm" onClick={applyAcl} disabled={saving()}>
                    {saving() ? t("common.saving") : t("objectSettings.applyAcl")}
                  </Button>
                </div>
              </div>
            </Show>
              ...
              <div class="flex flex-col gap-3">
                ...
                <Field label={t("objectSettings.restoreDays")}>
                  <Input
                    ...
                    min={1}
                    value={restoreDays()}
                    onInput={(e) => setRestoreDays(parseInt(e.currentTarget.value) || 1)}
                  />
                </Field>
                <Field label={t("objectSettings.restoreTier")}>
                  <SimpleSelect
                    value={restoreTier()}
                    options={RESTORE_TIERS.map((r) => ({ value: r, label: r }))}
                    onChange={setRestoreTier}
                  />
                </Field>
                <div class="flex justify-end">
                  <Button size="sm" onClick={doRestore} disabled={saving()}>
                    {saving() ? t("common.saving") : t("objectSettings.restore")}
                  </Button>
                </div>
              </div>
            </Show>
