...
export const BucketSettingsDialog: Component<{
  ...
  const [enc, setEnc] = createSignal<BucketEncryption>({
    enabled: false,
    sseAlgorithm: "AES256",
    kmsKeyId: "",
    bucketKeyEnabled: false,
  } as BucketEncryption);
  const [pab, setPab] = createSignal<PublicAccessBlock>({
    configured: false,
    blockPublicAcls: false,
    ignorePublicAcls: false,
    blockPublicPolicy: false,
    restrictPublicBuckets: false,
  } as PublicAccessBlock);
  ...
  const savePolicy = () =>
    run(() => BucketService.PutPolicy(props.connId, props.bucket, policy()) as Promise<void>);
  const saveCors = () =>
    run(() => BucketService.PutCORS(props.connId, props.bucket, cors() as any) as Promise<void>);
  const saveLifecycle = () =>
    run(
      () =>
        BucketService.PutLifecycle(props.connId, props.bucket, lifecycle() as any) as Promise<void>
    );
  const saveEncryption = () =>
    run(
      () => BucketService.PutEncryption(props.connId, props.bucket, enc() as any) as Promise<void>
    );
  const savePab = () =>
    run(
      () =>
        BucketService.PutPublicAccessBlock(props.connId, props.bucket, {
          ...pab(),
          configured: true,
  ...
  const saveTags = () =>
    run(
      () =>
        BucketService.PutTags(
          props.connId,
          props.bucket,
          tags().filter((x) => x.key.trim()) as any
        ) as Promise<void>
    );
          ...
          <div class="min-h-[18rem] py-1 text-sm">
                ...
                <div class="flex items-center justify-between rounded-md border p-3">
                  <div>
                    ...
                    <div class="text-xs text-muted-foreground">
                      {t("bucketSettings.versioningHint")}
                    </div>
                  </div>
            ...
            <Show when={tab() === "cors"}>
              <Section onSave={saveCors}>
                <For
                  each={cors()}
                  fallback={<p class="text-muted-foreground">{t("bucketSettings.corsEmpty")}</p>}
                >
                  {(rule, i) => (
                    <div class="flex flex-col gap-2 rounded-md border p-3">
                      <div class="flex items-center justify-between">
                        ...
                        <Button
                          variant="ghost"
                          size="icon-sm"
                          onClick={() => setCors((p) => p.filter((_, idx) => idx !== i()))}
                        >
                          <Trash2 class="h-4 w-4 text-destructive" />
                        </Button>
                      </div>
                      <CsvField
                        label={t("bucketSettings.allowedOrigins")}
                        value={rule.allowedOrigins}
                        onChange={(v) =>
                          setCors((p) =>
                            p.map((r, idx) => (idx === i() ? { ...r, allowedOrigins: v } : r))
                          )
                        }
                      />
                      <CsvField
                        label={t("bucketSettings.allowedMethods")}
                        value={rule.allowedMethods}
                        onChange={(v) =>
                          setCors((p) =>
                            p.map((r, idx) => (idx === i() ? { ...r, allowedMethods: v } : r))
                          )
                        }
                      />
                      <CsvField
                        label={t("bucketSettings.allowedHeaders")}
                        value={rule.allowedHeaders}
                        onChange={(v) =>
                          setCors((p) =>
                            p.map((r, idx) => (idx === i() ? { ...r, allowedHeaders: v } : r))
                          )
                        }
                      />
                      <CsvField
                        label={t("bucketSettings.exposeHeaders")}
                        value={rule.exposeHeaders}
                        onChange={(v) =>
                          setCors((p) =>
                            p.map((r, idx) => (idx === i() ? { ...r, exposeHeaders: v } : r))
                          )
                        }
                      />
                        ...
                        <Input
                          ...
                          onInput={(e) =>
                            setCors((p) =>
                              p.map((r, idx) =>
                                idx === i()
                                  ? { ...r, maxAgeSeconds: parseInt(e.currentTarget.value) || 0 }
                                  : r
                              )
                            )
                          }
            ...
            <Show when={tab() === "lifecycle"}>
              <Section onSave={saveLifecycle}>
                <For
                  each={lifecycle()}
                  fallback={
                    <p class="text-muted-foreground">{t("bucketSettings.lifecycleEmpty")}</p>
                  }
                >
                  {(rule, i) => {
                    ...
                    return (
                      <div class="flex flex-col gap-2 rounded-md border p-3">
                        <div class="flex items-center justify-between gap-2">
                          <Input
                            class="max-w-[14rem]"
                            placeholder={t("bucketSettings.ruleId")}
                            value={rule.id}
                            onInput={(e) => upd({ id: e.currentTarget.value })}
                          />
                          <div class="flex items-center gap-2">
                            ...
                            <Button
                              variant="ghost"
                              size="icon-sm"
                              onClick={() => setLifecycle((p) => p.filter((_, idx) => idx !== i()))}
                            >
                              <Trash2 class="h-4 w-4 text-destructive" />
                            </Button>
                          </div>
                        ...
                        <NumField
                          label={t("bucketSettings.prefix")}
                          text
                          value={rule.prefix}
                          onText={(v) => upd({ prefix: v })}
                        />
                        <NumField
                          label={t("bucketSettings.expireDays")}
                          value={rule.expirationDays}
                          onNum={(v) => upd({ expirationDays: v })}
                        />
                        <NumField
                          label={t("bucketSettings.noncurrentDays")}
                          value={rule.noncurrentVersionExpirationDays}
                          onNum={(v) => upd({ noncurrentVersionExpirationDays: v })}
                        />
                        <NumField
                          label={t("bucketSettings.abortMultipartDays")}
                          value={rule.abortIncompleteMultipartDays}
                          onNum={(v) => upd({ abortIncompleteMultipartDays: v })}
                        />
                        <div class="grid grid-cols-[10rem_1fr_1fr] items-center gap-2">
                          <Label>{t("bucketSettings.transition")}</Label>
                          <Input
                            type="number"
                            placeholder={t("bucketSettings.days")}
                            value={rule.transitionDays}
                            onInput={(e) =>
                              upd({ transitionDays: parseInt(e.currentTarget.value) || 0 })
                            }
                          />
                          <SimpleSelect
                            value={rule.transitionStorageClass}
                            placeholder={t("bucketSettings.storageClass")}
                            options={[
                  ...
                  onClick={() =>
                    setLifecycle((p) => [
                      ...p,
                      {
                        ...
                        abortIncompleteMultipartDays: 0,
                        transitionDays: 0,
            ...
            <Show when={tab() === "encryption"}>
              <Section onSave={saveEncryption}>
                <div class="flex items-center justify-between rounded-md border p-3">
                  ...
                  <Switch
                    checked={enc().enabled}
                    onChange={(v) => setEnc((p) => ({ ...p, enabled: v }) as BucketEncryption)}
                  />
                </div>
                <Show when={enc().enabled}>
                  <div class="grid grid-cols-[10rem_1fr] items-center gap-2">
                    ...
                    <SimpleSelect
                      value={enc().sseAlgorithm}
                      options={[
                        { value: "AES256", label: "AES256 (SSE-S3)" },
                        { value: "aws:kms", label: "aws:kms (SSE-KMS)" },
                      ]}
                      onChange={(v) =>
                        setEnc((p) => ({ ...p, sseAlgorithm: v }) as BucketEncryption)
                      }
                    />
                  </div>
                  <Show when={enc().sseAlgorithm === "aws:kms"}>
                    <div class="grid grid-cols-[10rem_1fr] items-center gap-2">
                      ...
                      <Input
                        ...
                        onInput={(e) =>
                          setEnc(
                            (p) => ({ ...p, kmsKeyId: e.currentTarget.value }) as BucketEncryption
                          )
                        }
                      />
                    </div>
                  ...
                  <div class="flex items-center justify-between rounded-md border p-3">
                    ...
                    <Switch
                      checked={enc().bucketKeyEnabled}
                      onChange={(v) =>
                        setEnc((p) => ({ ...p, bucketKeyEnabled: v }) as BucketEncryption)
                      }
                    />
                  </div>
                </Show>
                ...
                >
                  {([field, label]) => (
                    <div class="flex items-center justify-between rounded-md border p-3">
                      ...
                      <Switch
                        checked={!!pab()[field]}
                        onChange={(v) => setPab((p) => ({ ...p, [field]: v }) as PublicAccessBlock)}
                      />
                    </div>
                  )}
                </For>
            ...
            <Show when={tab() === "tags"}>
              <Section onSave={saveTags}>
                <For
                  each={tags()}
                  fallback={<p class="text-muted-foreground">{t("tags.empty")}</p>}
                >
                  {(tg, i) => (
                    <div class="grid grid-cols-[1fr_1fr_auto] gap-2">
                      <Input
                        ...
                        onInput={(e) =>
                          setTags((p) =>
                            p.map((x, idx) =>
                              idx === i() ? { ...x, key: e.currentTarget.value } : x
                            )
                          )
                        }
                      />
                      <Input
                        ...
                        onInput={(e) =>
                          setTags((p) =>
                            p.map((x, idx) =>
                              idx === i() ? { ...x, value: e.currentTarget.value } : x
                            )
                          )
                        }
                      />
                ...
                <Button
                  ...
                  size="sm"
                  class="self-start"
                  onClick={() => setTags((p) => [...p, { key: "", value: "" } as Tag])}
                >
                  <Plus class="h-4 w-4" />
                  {t("tags.add")}
                </Button>
...
const CsvField: Component<{ label: string; value: string[]; onChange: (v: string[]) => void }> = (
  p
) => (
  <div class="grid grid-cols-[10rem_1fr] items-center gap-2">
    <Label>{p.label}</Label>
...
const NumField: Component<{
  ...
  <div class="grid grid-cols-[10rem_1fr] items-center gap-2">
    ...
    <Input
      ...
      onInput={(e) =>
        p.text ? p.onText?.(e.currentTarget.value) : p.onNum?.(parseInt(e.currentTarget.value) || 0)
      }
    />
  </div>
);
